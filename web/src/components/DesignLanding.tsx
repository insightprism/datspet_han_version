...
/**
 * <DesignLanding> — the "Design a pet" front door (SPEC_PET_DESIGNER_PLATFORM §2).
 * Pure navigation + merchandising: one tile per "world" (each catalog animal with
 * a themed page) plus General, always present as the long-tail / power path. The
 * tile list is DATA-DRIVEN from /api/catalog (§4) — adding "Dragon Lair" is a
 * catalog entry + a themed shell, never a change here.
 *
 * This is the app's HOME: it renders at both `/` (standalone entry) and `/design`
 * (the DPP launch deep-link target, unchanged so the host needs no edit). Extracted
 * into one component so those two routes share it verbatim — no duplication.
 * "Describe a pet instead" → /make (the free-text Make-your-own page).
 */
