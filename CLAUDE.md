...
- **The themed pages, `/make`, and `components/PetDesigner.tsx` are DELETED** (`SPEC_PET_DESIGNER_FLOW` §11), and with them the whole legacy `/api/generate` contract. `SPEC_PET_DESIGNER_PLATFORM` §3 still describes the themed pages and is **substantially out of date** — do not treat it as current until it is revised (§11.3). Premade pets are the DB-backed **Pet Store** (`SPEC_PET_STORE`): `/catalog` is the shop page (search + tag filters), `webui/pet_store.py` the public browse/adopt surface, `webui/store_admin.py` the admin (stock prod at runtime — publish-from-pet copies an admin's house pet into `intake` with EMPTY listing text; the ✨ ai-tag door writes description+tags only when an admin asks and confirms, never as a side effect of stocking — SPEC_PET_STORE §4). Donations (§10) land in the same `intake` inbox and use those same Phase 1 admin routes. The old file-sample endpoints are retired (§8).
