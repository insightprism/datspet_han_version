...
the roll — identity survives a rebalance.
"""
import hashlib
import io
import json
...
def _sheet_in(zip_bytes):
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
        return next(z.read(n) for n in z.namelist() if n.endswith("_sprite.png"))
...
def test_stamped_block_matches_the_pets_own_facts():
    ...
    # §5.2 — the roll is the sheet-bytes derivation, so a stamped and an
    # unstamped copy of the same bundle are the same athlete.
    assert block["roll"] == athletics.derive_roll_from_sheet(_sheet_in(zip_bytes))
    # §3.1 — attributes come from the declared movement_class row (+ roll).
    row = athletics.base_row("aquatic_swimmer")
    assert block["water"] == row["water"]
    assert abs(block["speed"] - (row["speed"] + block["roll"])) < 1e-12
...
def test_every_other_manifest_key_passes_through_untouched():
    ...
    _, manifest_json = pet_athletics.stamp_pet_athletics(zip_bytes, at=MINTED_AT)
...
def test_stamp_survives_ownership_stamps_in_any_order():
    ...
    a, _ = pet_athletics.stamp_pet_athletics(zip_bytes, at=MINTED_AT)
    ...
    b, _ = pet_ownership.transfer_pet_ownership(
        ...
        name=pet_ownership.FACTORY_OWNER_NAME, at=MINTED_AT)
    b, b_manifest = pet_athletics.stamp_pet_athletics(b, at=MINTED_AT)
...
def test_restamp_is_a_same_object_noop():
    zip_bytes, _ = make_bundle_zip(animations=ANIMS,
                                   movement_class="mammalian_quadruped")
    once, once_manifest = pet_athletics.stamp_pet_athletics(zip_bytes, at=MINTED_AT)
    twice, twice_manifest = pet_athletics.stamp_pet_athletics(
        once, at="2027-01-01T00:00:00Z")
...
def test_remint_under_bumped_table_version_preserves_roll():
    ...
    block = json.loads(manifest_json)["athletics"]
    assert block["table_version"] == athletics.TABLE_VERSION
    assert block["roll"] == 0.05, "identity must survive the rebalance"
    row = athletics.base_row("mammalian_quadruped")
    assert abs(block["speed"] - min(row["speed"] + 0.05, 1.0)) < 1e-12


def test_a_bad_minted_at_raises_at_the_call_site():
    zip_bytes, _ = make_bundle_zip(animations=ANIMS)
    try:
        pet_athletics.stamp_pet_athletics(zip_bytes, at="not-a-timestamp")
