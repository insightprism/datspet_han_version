...
#: The bundle member the stamp patches — same contract as pet_ownership:
#: every other member is carried through byte-for-byte under its original name.
...
def set_pet_athletics(manifest_json: str, *, pet_id: str, at: str) -> str:
    ...
    reusing its stored identity nudges, so identity survives the rebalance
    (§5.3).
    ...
    """
    if not pet_id:
        raise AthleticsStampError("pet_id is required — it IS the identity (§3.4)")
    ...
    block = dict(athletics.resolve_athletics(manifest, pet_id))
...
def stamp_pet_athletics(zip_bytes: bytes, *, pet_id: str,
                        at: str) -> Tuple[bytes, str]:
    """`set_pet_athletics` applied to a bundle — patches manifest.json, carries
    every other member across byte-for-byte.
    ...
    after = set_pet_athletics(before, pet_id=pet_id, at=at)
    if after is before:
