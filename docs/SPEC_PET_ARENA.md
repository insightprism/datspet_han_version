...
>
> The owner: *"if the animal hits the hurdle, show a crash; 3 crashes ends the game as it is
> disqualified. also make the hurdle look like a real hurdle (solid color, mostly white), and make
> it move consistently."*
>
> **A crash is a missed JUMP question** — the wrong answer while parked at a gate IS hitting the
> hurdle, so the rule needs no new physics: the pet tumbles (its fallen pose, 💥 at the obstacle,
> the lane flashes), a counter ticks, and the THIRD crash disqualifies: the run ends immediately
> and the results row reads "💥 Disqualified — 3 crashes", ranked below every finisher and
> non-finisher, no medal. Rev.9's "no penalty" stance is retired by owner call. The refereed LOG
> stays pure (§7.4) — a crash emits nothing; disqualification is carried beside the accuracy
> tally, not inside the impulse stream. The bot cannot crash: it never answers wrong, and its pace
> rung remains its only knob.
>
> Presentation in the same pass: hurdles are DRAWN (white top board on two legs — athletics
> style), and the chase gains a speed CEILING beside its floor, so motion is velocity-banded —
> steady rolling speeds rather than rubber-band spikes.

<details><summary>Rev.10 — three choices: guessing must LOSE, it need not be impossible</summary>
>
> The owner, reversing Rev.1–9's typed-answer rule for mobile: *"the game has to be mobile
> friendly — typing in the answer is difficult. a better method would be 3 multiple choice
> answers… i can understand not rewarding guesses, but when a student guesses, he will have a high
> error rate, and he will not be able to beat someone who knows the answer. his drive to want to
> win will force him to improve on his own."*
>
> Adopted — this is the §8.5 tripwire firing exactly as written ("a later 'make it easier on
> mobile' cannot QUIETLY become multiple choice"): the reversal is loud, owner-made, and §8.5 is
> rewritten around the owner's own logic. Two teeth keep "guessing loses" true in the mechanics
> rather than by hope:
>
> **1. A miss burns the question.** A fresh question (and fresh choices) appears after every wrong
> tap — there is no eliminating your way to the answer; every guess is a fresh 1-in-3.
>
> **2. A missed tap freezes longer than a typo did** (`WRONG_CHOICE_LOCKOUT_MS`, 1.5 s vs the
> typed 0.6 s): a pure guesser pays ~2 misses per hit ≈ 4 s per answer against a knowing child's
> ~1 s — guessing runs at a QUARTER speed, which is the owner's error-rate argument as arithmetic.
>
> Choices are generated by the challenge from the same per-index seed (Rev.9), so both players see
> identical choices in identical order (§8.3). Typing keeps its keyboard — typing IS its skill;
> tap stays the floor (§8.6). §14's guessing guard re-pins to the new contract.

</details>

<details><summary>Rev.9 — hurdles gate: the jump is a harder question, in the jump color</summary>
>
...
**Crashes (Rev.11, owner rule):** a wrong answer while parked at a gate IS hitting the hurdle —
the pet tumbles into its fallen pose with a 💥 at the obstacle, and `HURDLE_CRASHES_TO_DQ` (3)
crashes disqualify the run on the spot. DQ is a *screen* outcome carried with the accuracy tally,
never encoded in the impulse log (§7.4 purity); the results row reads "💥 Disqualified" and ranks
below every finisher and non-finisher, no medal.
...
### 8.5 Guessing must lose — it need not be impossible (Rev.10)

**Three choices, tap the right one.** The owner's mobile call, reversing Rev.1–9's typed-answer
rule: an OS keyboard covers the track and makes answering miserable on a phone, and the owner's
own logic covers the guessing worry — *a guesser's error rate is the penalty, and the drive to win
forces improvement.* The mechanics make that logic literally true:

1. **A miss burns the question** — a fresh question with fresh choices appears, so there is no
   eliminating options; every guess is a fresh 1-in-3.
2. **A miss freezes longer than a typo did** — `WRONG_CHOICE_LOCKOUT_MS` (1.5 s) vs the typed
   0.6 s. A pure guesser pays ~2 misses per hit ≈ 4 s per answer against a knowing child's ~1 s:
   guessing runs at a quarter speed. If those two constants ever drift to where mashing
   approaches knowing, §14's guard is the alarm.

Decoys are the challenge's job (it knows what a *plausible* near-miss is — off-by-one, off-by-ten)
and are generated from the same per-index seed as the question (Rev.9), so every entrant sees
**identical choices in identical order** (§8.3). Typing keeps its keyboard — typing IS its skill —
and tap remains the floor (§8.6).
...
- **No backwards movement:** a run of only wrong answers leaves position at exactly the start line,
  never behind it (§7.2).
- **Guessing loses (Rev.10):** `arithmetic` deals exactly `NUM_ANSWER_CHOICES` (3) choices; the
  answer is among them; decoys are distinct from it and each other; the same `(seed, index, rung)`
  deals identical choices in identical order (§8.3); a miss burns the question (no elimination);
  and `WRONG_CHOICE_LOCKOUT_MS ≥ 2 × WRONG_ANSWER_LOCKOUT_MS` — the constant relation that keeps
  a masher at a fraction of a knowing child's pace (§8.5).
...
- **No adaptive difficulty** (§8.7), **no choice-elimination** (a missed choice burns the
  question — §8.5, Rev.10), and **no answer streak bonuses** — a streak multiplier compounds an
  early lead into an unrecoverable one, which is the same runaway-loser failure §7.2 avoids.
